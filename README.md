...
| Page | Description |
|------|-------------|
| **Page 1 - Email Tracking Report** | The primary dashboard. Displays all email send records from the tracking DE with filters, summary stats, a data table, and a CSV export. |
| **Page 2 - Bounce Detail Report** | Opened when a user clicks any Job ID on the main page. Shows a full breakdown of bounces for that specific send — by subcategory, count, percentage, and individual email addresses with their domains. |
...
| DE Name | Purpose |
|---------|---------|
| `Test_Email_Tracking_Report_DE` | Stores aggregated send-level metrics per Job ID |
| `Test_Email_Bounce_Detail_DE` | Stores individual bounce records with email addresses |
| `Test_Not_Sent_Tracking_DE` | Stores not-sent records including List Detective exclusions |

---

## How to Use

1. Create the three Data Extensions — `Test_Email_Tracking_Report_DE`, `Test_Email_Bounce_Detail_DE`, and `Test_Not_Sent_Tracking_DE` — in your SFMC BU with the appropriate fields matching the SQL column names
2. Set up the **`Test_Email_Tracking_Report_Refresh`** automation with the two SQL query activities (`Test_Populate_Email_Tracking_Report_DE` and `Test_Populate_Email_Bounce_Detail_DE`) and schedule it to run daily (or as needed)
3. For List Detective data: add a Data Extract and File Import activity in the automation to populate `Test_Not_Sent_Tracking_DE`
4. Create two CloudPages (Landing Pages or Code Resources) and paste the respective page code into each
5. Update the `BOUNCE_PAGE_URL` variable in the main tracking page to point to the published URL of your Bounce Detail page
6. Publish both CloudPages
7. Access the main tracking page URL and use the search filters to load the report
...
A drill-down page linked directly from Page 1. Every Job ID in the main table is a clickable link — clicking it passes the Job ID and Event Date as URL parameters to the Bounce Detail page. The page then queries the `Test_Email_Bounce_Detail_DE` for that specific Job ID and renders a full breakdown: bounce subcategory summary with counts and percentages, visual bar indicators, and a complete list of bounced email addresses grouped by subcategory with domain tags. Both pages include a Download CSV button that exports all data including metadata and summary sections.
